...
def test_workspace_metadata_load_from_file_generates_new_metadata_if_not_exists(user_data_directory: Path) -> None:
    metadata = WorkspaceMetadata.load_from_file()

    # Ensure UUIDs are valid
    assert isinstance(metadata.owner_uuid, UUID)
    assert isinstance(metadata.workspace_uuid, UUID)

    # Ensure oidc key is valid
    serialization.load_pem_private_key(metadata.oidc_private_key.encode(), password=None)

    metadata_file = user_data_directory / "workspace.json"
    assert metadata_file.exists()

    saved_metadata = WorkspaceMetadata.model_validate_json(metadata_file.read_text())
    assert saved_metadata == metadata


def test_workspace_metadata_load_from_file_loads_existing_metadata(
    user_data_directory: Path, oidc_private_key: str
) -> None:
    owner_uuid = uuid4()
    ...
    expected_metadata = WorkspaceMetadata(
